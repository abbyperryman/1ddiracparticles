...
def plotOnBlochSphere(x0, tmax):
  fig = plt.figure(figsize=(10,10))
  ax = fig.add_subplot(projection='3d')
  sphere = ax.plot_surface(X1, Y1, Z1, color = 'w', linewidth=0, antialiased=False, alpha=0.2)
  plt.title('Trajectories on the Bloch Sphere\n' + title)
  D = 120 
  tmax = 4
  cols = plt.cm.copper(np.linspace(0, 1, D))
  t = np.linspace(0,tmax,D)
  xp = odeint(v,x0,t)
  R = []
  T = []    
  O = []
  P = []
  for iter in range(D):
    vars = polar_decomp(xp[iter],t[iter])
    R.append(vars[0])
    T.append(vars[1])
    O.append(vars[2])
  print("Final theta value: %f. Theta at p = hk: %f" % (T[-1], arctan(m_el/k)))
  print("Final omega is:" + str(O[-1]))
  X = sin(T)*cos(O)
  Y = sin(T)*sin(O)
  Z = cos(T)
  for i in range(D-1):
    ax.plot(X[i:i+2],Y[i:i+2],Z[i:i+2], color = cols[i])
  plt.show()

# Here we compute a number of trajectories and show their finals positions on the Bloch sphere.

def plotFinalPosOnBlochSphere(x0, t, N):
  D = 120
  tmax = t
  t = np.linspace(0,tmax,tmax+1)
  initials = np.random.normal(0, sigma, N)
  cols = plt.cm.copper(np.linspace(0, 1, D))
  Rf = []
  Tf = []
  Of = []
  ...
  ax = fig.add_subplot(projection='3d')
  sphere = ax.plot_surface(X1, Y1, Z1, color = 'w', linewidth=0, antialiased=False, alpha=0.2)
  plt.title('Bloch sphere location after ' + str(tmax) + ' seconds\n' + title)
  ax.scatter(X,Y,Z, c = cols)
  plt.show()


def plotTimeEvolutionofBlochVars(x0, t):
  D = 120
  tmax = t
  t = np.linspace(0,tmax, D)
  xp = odeint(v,x0,t)
  ...
  for iter in range(D):
    ...
    O.append(vars[2])
    P.append(vars[3])
  plt.title("R variable\n" + title)
  plt.xlabel("Time")
  plt.ylabel("R")
  plt.plot(t, R, color="red")
  plt.show()
  plt.title("Theta variable\n" + title)
  plt.xlabel("Time")
  plt.ylabel("Theta")
  plt.plot(t, T, color="blue")
  plt.show()
  plt.title("Omega variable\n" + title)
  plt.xlabel("Time")
  plt.ylabel("Omega")
  plt.plot(t, O, color="green")
  plt.show()
  plt.title("Phi variable\n" + title)
  plt.xlabel("Time")
  plt.ylabel("Phi")
  ...
  Pf = P[-1]
  finalp = m_el/tan(Tf)
  print("Final R value:", Rf)
  print("Final Theta value:", Tf)
  print("Final Omega value:", Of)
  print("Final Phi value:", Pf)
  print("Final momentum:", finalp)
